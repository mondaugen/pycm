...
#	return generate_random_form_from_distinctness(8,3)
#    if depth <= 2:
#	return generate_random_form_from_distinctness(2,2)
...
#print length, number of formal elements per track, number of tracks
pdict = list(set(parts))
print repr(int(gcd.denominator)), repr(len(formstrs)), repr(len(tracks))

# a 2d array indexed by (tracknumber, element number). Length will be stored at
# the entry
trelems = [[[] for y in xrange(len(formstrs))] for x in xrange(len(tracks))]

print repr(len(formstrs)*len(tracks))
for i in xrange(len(seqary)):
    for j in xrange(len(seqary[i])):
	x, y = seqary[i][j]
	trelems[what_track(x,tracks)][formstrs.index(strip_track(x))]\
		= y

for i in xrange(len(tracks)):
    for j in xrange(len(formstrs)):
	print i, j, repr(trelems[i][j])
