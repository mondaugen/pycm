...
def my_depth_adjust(depth,curnode):
    if depth == 3:
	if (curnode.parent.name == 'A') & my_depth_adjust.conta:
	    my_depth_adjust.conta = False
	    return depth + 1
    if depth == 2:
	if (curnode.parent.name == 'B') & my_depth_adjust.conta:
	    my_depth_adjust.conta = False
	    return depth + 1
    return depth
my_depth_adjust.conta = True
my_depth_adjust.contb = True

def another_vert_generator(depth,curnode):
    column = []
    if (depth <= (3)) & my_depth_adjust.conta :
	for i in xrange(2):
