...
def new_depth_adjust(depth,curnode):
    if depth == 2:
	if curnode.name == 'a' and new_depth_adjust.continuea:
	    new_depth_adjust.continuea = False
	    return depth + 1
	if curnode.name == 'b' and new_depth_adjust.continueb:
	    new_depth_adjust.continueb = False
	    return depth + 3
    return depth
new_depth_adjust.continuea = True
new_depth_adjust.continueb = True

def another_vert_generator(depth,curnode):
    column = []
    if ((depth <= 2) and curnode.name == 'a' )\
	    or (depth == startdepth):
	if new_depth_adjust.continuea:
	    for i in xrange(2):
		column.append(Sequential(chr(ord('A') + i)))
    if ((depth <= 2) and curnode.name == 'b' )\
	    or (depth == startdepth):
	if new_depth_adjust.continueb:
	    for i in xrange(2):
		column.append(Sequential(chr(ord('A') + i)))
    else:
...
s.grow_vh_tree_w_lengths(startdepth, temp_form_gen,\
	another_vert_generator, temp_len_gen, new_depth_adjust)
...
print repr(int(gcd.denominator)), repr(len(pdict)), repr(len(tracks))
